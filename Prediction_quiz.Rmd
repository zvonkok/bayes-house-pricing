...
AT <- ames_test %>% mutate(price = log(price))

P <- predict(model.AIC, AT)
...
predict.full <- exp(predict(model.full, ames_train))
...
<li> <b>When predicting to `ames_train`, the RMSE for `model.AIC` is higher than the RMSE for `model.full`.  However, when predicting to `ames_test`, the RMSE for `model.full` is higher.</b>
